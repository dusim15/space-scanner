...
class PathPlanner:
    ...
    def __init__(self, config_manager, event_system):
        ...
        # Add new attributes for dynamic replanning
        self.current_position = None
        self.target_position = None
        self.obstacle_detection_radius = 1.0  # meters
        self.replan_threshold = 0.5  # meters
        self.last_replanning_time = 0
        self.replan_cooldown = 0.5  # seconds
    ...
    def update_dynamic_obstacles(self, point_cloud, current_position):
        """Update occupancy grid with new obstacle information"""
        try:
            self.current_position = current_position
            
            # Get points near the current path
            if self.path is not None:
                path_points = np.array(self.path)
                current_idx = np.argmin(np.linalg.norm(path_points - current_position, axis=1))
                remaining_path = path_points[current_idx:]
                
                # Check for new obstacles near the remaining path
                points = np.asarray(point_cloud.points)
                tree = KDTree(points)
                
                new_obstacles_detected = False
                for path_point in remaining_path:
                    # Find points within detection radius of path
                    indices = tree.query_ball_point(path_point, self.obstacle_detection_radius)
                    if indices:
                        # Convert nearby points to voxel coordinates
                        voxel_coords = np.floor(points[indices] / self.config.get('scanning', 'voxel_size')).astype(int)
                        
                        # Check for new obstacles
                        for voxel in voxel_coords:
                            voxel_tuple = tuple(voxel)
                            if voxel_tuple not in self.occupancy_grid:
                                self.occupancy_grid[voxel_tuple] = True
                                new_obstacles_detected = True
                
                # Trigger replanning if new obstacles detected
                if new_obstacles_detected:
                    current_time = time.time()
                    if current_time - self.last_replanning_time > self.replan_cooldown:
                        self.replan_path(current_position)
                        self.last_replanning_time = current_time
                
                return new_obstacles_detected
                
        except Exception as e:
            logger.error(f"Error updating dynamic obstacles: {e}")
            return False

    def replan_path(self, current_position):
        """Replan path from current position to target"""
        try:
            if self.target_position is None:
                return False
                
            logger.info("Replanning path due to detected obstacles")
            
            # Update navigation graph with new obstacle information
            self.build_navigation_graph()
            
            # Find new path from current position to target
            new_path = self.find_path(current_position, self.target_position)
            
            if new_path is not None:
                self.path = new_path
                # Publish path update event
                self.events.publish('path_updated', new_path)
                logger.info("Path replanned successfully")
                return True
            else:
                logger.warning("Could not find valid path during replanning")
                return False
                
        except Exception as e:
            logger.error(f"Error replanning path: {e}")
            return False
...
class LocalizationSystem:
    """Handles real-time position tracking using various methods"""
    ...
    def __init__(self, config_manager, event_system):
        ...
        self.tracking_methods = {
            'visual_odometry': self._visual_odometry,
            'imu': self._imu_tracking,
            'aruco': self._aruco_tracking,
            'feature_matching': self._feature_matching
        }
        
        # Select primary and backup tracking methods
        self.primary_method = 'visual_odometry'
        self.backup_method = 'feature_matching'
        
    def update_position(self, frame, imu_data=None):
        """Update current position using available tracking methods"""
        try:
            # Try primary tracking method
            new_position = self.tracking_methods[self.primary_method](frame, imu_data)
            
            if new_position is None:
                # Fallback to backup method if primary fails
                logger.warning(f"Primary tracking method {self.primary_method} failed, using {self.backup_method}")
                new_position = self.tracking_methods[self.backup_method](frame, imu_data)
            
            if new_position is not None:
                self.position = new_position
                self.events.publish('position_updated', self.position)
    ...
    def _visual_odometry(self, frame, _):
        """Track position using visual odometry"""
        try:
            ...
            if M is not None:
                # Extract translation
                translation = M[:2, 2]
                # Update position (scale factor would need to be calibrated for real-world units)
                scale = 0.1  # Example scale factor
                self.position[:2] += translation * scale
    ...
    def _imu_tracking(self, _, imu_data):
        """Track position using IMU data"""
        try:
            if imu_data is None:
                return None
            ...
            self.position += linear_accel * dt * dt / 2
            
            return self.position
            
        except Exception as e:
            logger.error(f"Error in IMU tracking: {e}")
            return None
            
    def _aruco_tracking(self, frame, _):
        """Track position using ArUco markers"""
        try:
            ...
            if ids is not None:
                # Get camera matrix and distortion coefficients (should be calibrated)
                ...
                position = np.mean(tvecs, axis=0)[0]
            ...
            logger.error(f"Error in ArUco tracking: {e}")
            return None
            
    def _feature_matching(self, frame, _):
        """Track position using feature matching"""
        try:
            ...
            matches = flann.knnMatch(self.previous_descriptors, descriptors, k=2)
            ...
            if len(good_matches) > 10:
                # Get matched keypoints
                src_pts = np.float32([self.previous_keypoints[m.queryIdx].pt for m in good_matches])
...
class RoomMapper:
    ...
    def __init__(self):
        ...
        self.localization_system = LocalizationSystem(self.config_manager, self.event_system)
        
        # Initialize advanced systems
        self.perception_system = AdvancedPerceptionSystem(
            self.config_manager, 
            self.event_system
        )
        self.decision_system = ParallelDecisionSystem(
            self.config_manager, 
            self.event_system
        )
    ...
    def process_scan(self):
        """Enhanced process_scan with advanced perception"""
        try:
            # Get sensor data
            sensor_data = self._gather_sensor_data()
            
            # Process environment in parallel
            environment_analysis = self.perception_system.analyze_environment(sensor_data)
            
            # Make decisions based on analysis
            decisions = self.decision_system.process_environment(environment_analysis)
            
            # Update path planning if needed
            if decisions['update_path_required']:
                self.path_planner.replan_path(
                    current_position=self.get_current_position(),
                    environment_data=environment_analysis
                )
                
            # Update visualization with advanced data
            self._update_advanced_visualization(environment_analysis, decisions)
            
        except Exception as e:
            logger.error(f"Enhanced scan processing error: {e}")
    ...
    def get_current_position(self):
        ...
        try:
            ...
            if frames:
                ...
                # Get IMU data if available (implement based on your hardware)
                imu_data = self.get_imu_data()
                
                # Update and get position
                position = self.localization_system.update_position(left_frame, imu_data)
                return position
                
        except queue.Empty:
            return self.localization_system.position  # Return last known position
        except Exception as e:
